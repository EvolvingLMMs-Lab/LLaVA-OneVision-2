...
std::vector<at::Tensor> mlp_forward(int use_bias, int activation, std::vector<at::Tensor> inputs) {
  ...
  auto out = at::empty({batch_size, output_features.back()}, inputs[0].scalar_type());
  auto reserved_space = at::empty({static_cast<long>(reserved_size)}, inputs[0].scalar_type());
  // allocate fixed 4MB workspace for cublaslt for now, and this gets at least 4 MB
  auto lt_workspace = at::empty({1 << 22}, inputs[0].scalar_type());
...
std::vector<at::Tensor> mlp_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(inputs[0].scalar_type(), "mlp_backward", [&] {
    ...
    auto work_space = at::empty({static_cast<long>(work_size / sizeof(scalar_t))}, inputs[0].scalar_type());
